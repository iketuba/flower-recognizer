...
def main():
    ...
    tr_dataloader = torch.utils.data.DataLoader(tr_data, batch_size=batch_size, shuffle=True, )
    ...
    train_model(model_ft, tr_dataloader, val_dataloader, criterion, optimizer_ft, device, num_epochs=num_epoch, num_val=len(val_data))
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for phase in ['train', 'eval']:
            ...
            for (i, (inputs, labels)) in enumerate(dataloaders):
                ...
                if phase == 'train':
                    print('({}/{}) Epoch:{}/{} Loss:{:.5f} AveLoss:{:.5f}'.format(i, int(len(dataloaders)), epoch, num_epochs, loss.item(), running_loss/(i+1)))
                else:
                    print('phase:eval Loss:{:.5f} Accuracy:{:.5f}%'.format(loss.item(), running_corrects.item()/num_val*100))
            if (epoch)%5 == 0:
                print('save model')
